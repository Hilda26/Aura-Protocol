"""
Shared fixtures/workarounds for Aura direct-mode tests.
"""
...
def warp_to(direct_vm, iso: str) -> None:
    ...
    direct_vm.warp() alone only patches datetime.now(); it does not touch
    gl.message_raw / gl.message.raw, which are injected once at contract
    load and never refreshed by warp(). Bridge the gap here so time-
    dependent contract logic (reading gl.message.raw["datetime"]) actually
    observes the warped time in tests.
    """
    direct_vm.warp(iso)
